...
output: 
  html_document:
    ...
    toc_float:
      ...
      smooth_scroll: false
params:
  base_scen: 'C:/FTOT/scenarios/reference_scenarios/rs1_multi_commodity_supply_chain'
---
...
g1 <- ggplot(res, aes(x = disrupt_step, 
  ...
  theme_bw() +
  ggtitle(paste('Betweeness centrality disruption scenario:', scen_name))
...
The left panel colors in the points by the rank in terms of volume. The top links (used for the disruption test) are colored in.
The right panel colors int the points by the rank in terms of sum of between centrality.
...
g2_bc <- ggplot(e_j, aes(x = volume, 
  ...
  theme_bw() #+
  #ggtitle('Betweeness centrality sum and Volume of edges in optimal scenario solution')
...
datatable(e_j  %>% filter(!is.na(BC_rank) | !is.na(V_rank)) %>%
            select(BC_rank,
                   ...
                   sum_BC,
                   mode_oid),
          caption = paste("Road segements (edges) disrupted in", scen_name),
          rownames = F,
          filter = 'top',
