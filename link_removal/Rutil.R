...
for (i in loadpacks) {
  if (length(grep(i, (.packages(all.available=T, lib.loc=use_lib)))) == 0) {
    print(paste("<<>> Installing R package", i, "in", use_lib, "<<>>"))
    
    install.packages(i, dependencies = c("Depends", "Imports"), repos = "https://cloud.r-project.org", type = "binary", lib = use_lib)
